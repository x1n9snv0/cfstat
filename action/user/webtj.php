...
<table class="tb_1">
    ...
    while ($rs = mysqli_fetch_assoc($result2)) {
        ?>
        <tr class="tr_2">
            <td><a href="?action=urlgo&url=<?php echo urlencode($rs["weburl"]); ?>"
                   target="_blank"><?php echo htmlspecialchars($rs["weburl"]); ?></a></td>
...
<table class="tb_3">
    <tr>
        <td>
            ...
            <?php } ?>
            <a href="?action=<?php echo $action; ?>&adddate=<?php echo $adddate; ?>&px=<?php echo $px; ?>&page=<?php echo $totalpage; ?>">最后一页</a>&nbsp;共<font
