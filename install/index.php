...
function connopen()
{
    ...
    $conn = new mysqli($dbhost,$dbuser,$dbpwd) or die("对不起，发生错误！ 请检查conn.php中数据库的配置是否正确！");
...
if ($action == "dbconnsave") {
    ...
    if (!$conn) {
        ...
        if (!$db) {
            ...
            if (substr(mysqli_get_server_info($conn), 0, 1) == "4") {
                $sql = "CREATE DATABASE `$dbname`";
            } else {
                $sql = "CREATE DATABASE `$dbname` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci";
            }
            $conn->query($sql) or die("数据库$dbname不存在,系统创建的时候也失败");
        }
    }
    $conn->select_db($dbname) or die("选择数据库失败,请正确填写数据库名称信息!");
...
if ($action == "") {
    ...
    <table class="tb_2">
        ...
        <tr>
            <td class="td_2">
                <input name="Submit" type="button" class="button" onClick="window.location='?action=step_2'"
                       value="开始新安装" style="margin:10px 0px 80px">
            </td>
        </tr>
        <tr class="tr_1">
        ...
        <tr>
            <td class="td_2"><input name="Submit" type="button" class="button"
                                    onClick="{if(confirm('确定你已经安装过，需要生成安装锁定文件么?')){window.location='?action=setuplock';return true;}return false;}"
                                    value="已安装，生成安装锁定文件install.lock" style="margin:10px 0 20px">
                <br>
        ...
        $(function () {
            $("#submit").click(function () {
                ...
                $.ajax({
                    url: "?",
                    type: "post",
                    data: "action=dbconnsave&dbhost=" + dbhost + "&dbname=" + dbname + "&dbuser=" + dbuser + "&dbpwd=" + dbpwd,
...
//如果Mysql的版本为低版本时,不能使用DEFAULT CHARSET=utf8
    if (substr(mysqli_get_server_info($conn), 0, 1) == "4") $sql = str_replace(" DEFAULT CHARSET=utf8", "", $sql);
    ...
    <script>
        function check() {
            ...
            if (document.form1.pwd.value === "admin") {
                return (confirm('确定要使用默认的密码admin吗，确定则进行下一步，取消则重新输入密码?'))
            }
            return true;
        }
    </script>
